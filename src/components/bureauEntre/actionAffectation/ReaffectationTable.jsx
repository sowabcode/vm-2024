...
const ReaffectationTable = () => {
  ...
  const { patient, lastAffection, lastServiceId, reaffecterPatient, response } =
    useContext(PatientContext);

  useEffect(() => {
    if (patient && patient._id !== undefined) {
      lastAffection(patient._id);
    }
  }, [patient]);

  const handleReaffecterPatient = (departmentId) => {
    // console.log(departmentId);
    reaffecterPatient(departmentId, patient._id);
  };
    ...
    <div className="mt-5 space-y-2">
      ...
      {filterServices.map((service) => (
        <div
          key={service._id}
          className={`${
            service._id === lastServiceId
              ? "bg-green-200 flex items-center justify-between border px-2 py-1 rounded-md shadow-md"
              : "flex items-center justify-between border px-2 py-1 rounded-md shadow-md"
          }`}
        >
          ...
          <button
            onClick={() => handleReaffecterPatient(service._id)}
...
export default ReaffectationTable;
